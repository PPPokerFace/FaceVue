...
//                const webCam = new Webcam(this.$refs['myvideo']);
            ...
            runCam: async function (Camera, Canvas) {
                ...
                const ratio = Math.min(CANVAS_HEIGHT/video.videoHeight, CANVAS_WIDTH/video.videoWidth);
                const drawImage_Height = ratio * video.videoHeight;
                const drawImage_Width = ratio * video.videoWidth;
                const drawImage_X = (CANVAS_WIDTH - drawImage_Width) / 2;
                const drawImage_Y = (CANVAS_HEIGHT - drawImage_Height) / 2;
...
//                CanvasContext.globalCompositeOperation = 'source-atop';//重叠部分可见，其他透明。
                ...
                console.log("Canvas Size:", drawImage_Width, drawImage_Height);    // eslint-disable-line
                ...
                while (true) // eslint-disable-line
                {
                    ...
                    const boxes = await this._predict("v3_tiny_model", model, video, MAX_BOXES, SCORE_THRESHOLD, IOU_THRESHOLD, 1, v3_tiny_anchors, faceClasses, imageSize)
                    if (frameNum <= 50)
                        continue
                    ...
                    boxes.forEach(box => {
                        ...
                        CanvasContext.fillText(score.toFixed(2), _left, _top)
                        CanvasContext.strokeRect(_left, _top,width, height);
